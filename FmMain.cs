...
namespace TrOCR
{
	...
	public partial class FmMain : Form
	{
		...
		protected override void WndProc(ref Message m)
		{
			...
			{
				if (IniHelp.GetValue("工具栏", "Top") == "True")
				{
					TopMost = true;
					return;
				}
			...
			else
			{
				...
				if (m.Msg == 786 && m.WParam.ToInt32() == 580)
				{
					...
					auto_fla = "开启";
					split_txt = "";
					RichBoxBody.Text = "***No text found in this area***";
					RichBoxBody_T.Text = "";
					typeset_txt = "";
					transtalate_fla = "关闭";
				...
				if (m.Msg == 786 && m.WParam.ToInt32() == 590 && speak_copyb == "Read")
				{
					TTS();
					return;
				}
				...
				if (m.Msg != 163)
				{
					if (m.Msg == 786 && m.WParam.ToInt32() == 222)
					{
						...
						base.Show();
						WindowState = FormWindowState.Normal;
						if (IniHelp.GetValue("快捷键", "翻译文本") != "Please press the shortcut key")
						{
					...
					if (m.Msg == 786 && m.WParam.ToInt32() == 235)
					{
						if (!Visible)
						{
							...
							Visible = true;
							Thread.Sleep(100);
							if (IniHelp.GetValue("工具栏", "Top") == "False")
							{
								TopMost = false;
								return;
							}
		...
		public void OCR_baidu_bak()
		{
			...
			try
			{
				...
				memoryStream.Close();
				if (interface_flag == "CH_EN")
				{
					str = "CHN_ENG";
				}
				if (interface_flag == "JAP")
				{
					str = "JAP";
				}
				if (interface_flag == "KOR")
				{
					str = "KOR";
				}
				...
				var httpWebRequest = (HttpWebRequest)WebRequest.Create("https://aip.baidubce.com/rest/2.0/ocr/v1/general");
				...
				httpWebRequest2.Referer = "https://aip.baidubce.com/rest/2.0/ocr/v1/general";
		...
		{
			IniHelp.SetValue("配置", "Interface", interface_flag);
		}
		...
		public void readIniFile()
		{
			...
			if (Proxy_flag == "no proxy")
			{
				WebRequest.DefaultWebProxy = null;
			}
			if (Proxy_flag == "system proxy")
			{
				WebRequest.DefaultWebProxy = WebRequest.GetSystemWebProxy();
			}
			if (Proxy_flag == "custom proxy")
			{
				try
				{
				...
				{
					MessageBox.Show("请检查代理Settings！");
				}
			}
			interface_flag = IniHelp.GetValue("配置", "Interface");
			if (interface_flag == "发生错误")
			{
				IniHelp.SetValue("配置", "Interface", "Sougou");
				OCR_foreach("Sougou");
			}
			else
			{
			...
			if (IniHelp.GetValue("快捷键", "文字识别") != "Please press the shortcut key")
			{
			...
			if (IniHelp.GetValue("快捷键", "记录界面") != "Please press the shortcut key")
			{
			...
			if (IniHelp.GetValue("快捷键", "识别界面") != "Please press the shortcut key")
			{
			...
			StaticValue.baiduAPI_ID = HelpWin32.IniFileHelper.GetValue("Key_Baidu", "secret_id", filePath);
			if (HelpWin32.IniFileHelper.GetValue("Key_Baidu", "secret_id", filePath) == "发生错误")
			{
				StaticValue.baiduAPI_ID = "请输入secret_id";
			}
			StaticValue.baiduAPI_key = HelpWin32.IniFileHelper.GetValue("Key_Baidu", "secret_key", filePath);
			if (HelpWin32.IniFileHelper.GetValue("Key_Baidu", "secret_key", filePath) == "发生错误")
			{
		...
		public void tray_Set_Click(object sender, EventArgs e)
		{
			...
			if (fmSetting.DialogResult == DialogResult.OK)
			{
				...
				Proxy_name = IniHelp.GetValue("代理", "服务器账号");
				Proxy_password = IniHelp.GetValue("代理", "服务器密码");
				StaticValue.baiduAPI_ID = IniHelp.GetValue("Key_Baidu", "secret_id");
				StaticValue.baiduAPI_key = IniHelp.GetValue("Key_Baidu", "secret_key");
				if (Proxy_flag == "no proxy")
				{
					WebRequest.DefaultWebProxy = null;
				}
		...
		public void trans_Calculate()
		{
			...
			else
			{
				if (interface_flag == "KOR")
				{
					...
					RichBoxBody_T.set_language = "KOR";
				}
				if (interface_flag == "JAP")
				{
					StaticValue.zh_en = false;
					...
					RichBoxBody_T.set_language = "JAP";
				}
				if (interface_flag == "CH_EN")
				{
					...
					StaticValue.zh_jp = false;
					StaticValue.zh_ko = false;
					RichBoxBody_T.set_language = "CH_EN";
				}
				if (IniHelp.GetValue("配置", "翻译Interface") == "Google")
				{
					googleTranslate_txt = Translate_Google(typeset_txt);
				}
				if (IniHelp.GetValue("配置", "翻译Interface") == "Tencent")
				{
		...
		public void TTS_thread()
		{
			try
			{
				...
				ttsData = array2;
				if (speak_copyb == "Read" || voice_count == 0)
				{
			...
			catch (Exception ex)
			{
				...
				{
					MessageBox.Show("The text is too long, please use the right-click menu to select and read!", "提醒");
				}
			}
		}
		...
		public string Translate_Google(string text)
		{
			...
			{
				text2 = "[Google interface error]：\r\nNetwork error or text too long.";
			}
			return text2;
		}
		...
		public void SetHotkey(string text, string text2, string value, int flag)
		{
			...
			{
				fmflags.Show();
				fmflags.DrawStr("Shortcut key conflict, please replace!");
			}
				...
				try
				{
					...
					else
					{
						...
						if (mode_flag == "截图")
						{
							...
							HelpWin32.UnregisterHotKey(Handle, 222);
							StaticValue.截图排斥 = false;
							if (IniHelp.GetValue("截图音效", "Clipboard") == "True")
							{
								PlaySong(IniHelp.GetValue("截图音效", "音效路径"));
							}
							fmflags.Show();
							fmflags.DrawStr("Screenshot copied");
						}
		...
		public void Main_OCR_Thread()
		{
			...
			if (interface_flag == "JAP" || interface_flag == "CH_EN" || interface_flag == "KOR")
			{
				OCR_baidu();
				fmloading.fml_close = "窗体已关闭";
				Invoke(new ocr_thread(Main_OCR_Thread_last));
			}
			if (interface_flag == "Left to right" || interface_flag == "Right to left")
			{
				shupai_Right_txt = "";
				var image = image_screen;
		...
		public void Main_OCR_Thread_last()
		{
			...
			if (IniHelp.GetValue("配置", "识别弹窗") == "False")
			{
				...
				Visible = false;
				fmflags.Show();
				if (RichBoxBody.Text == "***No text found in this area***")
				{
					fmflags.DrawStr("无文本");
				}
				else
		...
		public void OCR_baidu()
		{
			...
			try
			{
				...
				if (baidu_vip == "")
				{
					MessageBox.Show("请检查Key输入是否正确！", "提醒");
				}
				else
				{
					var str = "CHN_ENG";
			...
			catch
			{
				if (esc != "Quit")
				{
					RichBoxBody.Text = "***No text found in this area或者Key次数用尽***";
				}
				else
				{
					RichBoxBody.Text = "***No text found in this area***";
		...
		private void OCR_foreach(string name)
		{
			...
			if (name == "Sougou")
			{
				interface_flag = "Sougou";
				Refresh();
				sougou.Text = "Sougou√";
			}
			if (name == "Tencent")
			{
				interface_flag = "Tencent";
				Refresh();
				tencent.Text = "Tencent√";
			}
			if (name == "Youdao")
			{
				interface_flag = "Youdao";
				Refresh();
				youdao.Text = "Youdao√";
			}
			if (name == "Formulas")
			{
				interface_flag = "Formulas";
				Refresh();
				Mathfuntion.Text = "Formulas√";
			}
			if (name == "BaiduTable")
			{
				interface_flag = "BaiduTable";
				Refresh();
				ocr_table.Text = "Table√";
				baidu_table.Text = "Baidu√";
			}
			if (name == "AliTable")
			{
				interface_flag = "AliTable";
				Refresh();
				ocr_table.Text = "Table√";
				ali_table.Text = "Ali√";
			}
			if (name == "Left to right")
			{
				...
				{
					interface_flag = "Left to right";
					Refresh();
					shupai.Text = "Vertical row√";
					left_right.Text = "Left to right√";
				}
			}
			if (name == "Right to left")
			{
				...
				interface_flag = "Right to left";
				Refresh();
				shupai.Text = "Vertical row√";
				righ_left.Text = "Right to left√";
			}
			HelpWin32.IniFileHelper.SetValue("配置", "Interface", interface_flag, filePath);
		}
		...
		{
			null_Proxy.Text = "no proxy√";
			customize_Proxy.Text = "custom proxy";
			system_Proxy.Text = "system proxy";
			Proxy_flag = "关闭";
			WebRequest.DefaultWebProxy = null;
		}
		...
		private void Trans_foreach(string name)
		{
			if (name == "Google")
			{
				trans_google.Text = "Google√";
				trans_tencent.Text = "Tencent";
				IniHelp.SetValue("配置", "翻译Interface", "Google");
			}
			if (name == "Tencent")
			{
				trans_google.Text = "Google";
				trans_tencent.Text = "Tencent√";
				IniHelp.SetValue("配置", "翻译Interface", "Tencent");
			}
		}
		...
		public string TencentPOST(string url, string content)
		{
			...
			{
				result = "[Tencent interface error]：\r\nPlease switch to another interface or try again。";
			}
			return result;
		}
		...
		private string Translate_Tencent(string strtrans)
		{
			...
			{
				text = "[Tencent interface error]：\r\n1.There is a problem with the interface request, waiting to be fixed。";
			}
			return text;
		}
		...
		public void OCR_baidu_table()
		{
			...
			{
				RichBoxBody.Text = "[消息]：免费BaiduKey50次已经耗完！请更换自己的Key继续使用！";
			}
		}
		...
		public void Main_OCR_Thread_table()
		{
			...
			if (interface_flag == "BaiduTable")
			{
				...
				RichBoxBody.Text = "[message]: Table copied to clipboard!";
		...
		public void OCR_ali_table()
		{
			...
			{
				RichBoxBody.Text = "[消息]：AliTable识别出错！";
